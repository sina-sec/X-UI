...
uninstall() {
    ...
    echo ""
    echo -e "The uninstallation is successful. If you want to delete this script, run it after exiting the script ${green}rm /usr/bin/x-ui -f${plain} delete"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/sina-sec/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
ssl_cert_issue() {
    ...
    LOGI "4.The script applies for a certificate. The default installation path is/root/cert directory"
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            LOGE "Modify the default CA to Lets'Encrypt failed, script exits"
            exit 1
        fi
        ...
        if [ $? -ne 0 ]; then
            ...
            LOGI "The certificate has been successfully issued and is being installed..."
        fi
...
————————————————
  ...
  ${green}16.${plain} 一key to apply for an SSL certificate (acme apply)
 "
    show_status
