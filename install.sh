...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/${arch}.tar.gz https://github.com/sina-sec/x-ui/releases/download/${last_version}
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/sina-sec/x-ui/releases/download/${last_version}
