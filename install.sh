...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/sina-sec/x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/${last_version}.tar.gz https://github.com/sina-sec/x-ui/archive/refs/tags/${last_version}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/sina-sec/x-ui/archive/refs/tags/0.3.3(amd64).tar.gz"
        echo -e "start installation x-ui v$1"
        wget -N --no-check-certificate -O /usr/local/*.tar.gz ${url}
        if [[ $? -ne 0 ]]; then
