...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/${arch}.tar.gz https://github.com/sina-sec/x-ui/releases/${last_version}/${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/sina-sec/x-ui/releases/${last_version}/${arch}.tar.gz"
        echo -e "start installation x-ui v$1"
        wget -N --no-check-certificate -O /usr/local/${arch}.tar.gz ${url}
        if [[ $? -ne 0 ]]; then
