...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/*.tar.gz https://github.com/sina-sec/x-ui/archive/refs/tags/${last_version}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/sina-sec/x-ui/releases/${last_version}/*.tar.gz"
